...
def qcewGetAreaData(year,qtr,area):
    urlPath = f"http://data.bls.gov/cew/data/api/{year}/{qtr.lower()}/area/{area.upper()}.csv"
...
def qcewGetIndustryData(year,qtr,industry):
    urlPath = f"http://data.bls.gov/cew/data/api/{year}/{qtr.lower()}/{industry/{IND}.csv"
    httpStream = urllib.request.urlopen(urlPath)
    csv = httpStream.read()
    httpStream.close()
    return qcewCreateDataRows(csv)
